...
def _learning_sign_item(token: str) -> Optional[Dict]:
...
def _validate_password_strength(password: str, email: Optional[str] = None) -> Optional[str]:
...
def validate_password(req: PasswordValidationRequest):
    ...
    validation_error = _validate_password_strength(req.password, req.email)
    if validation_error:
