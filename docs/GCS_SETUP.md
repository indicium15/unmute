...
gcloud run services update unmute-backend \
  --set-env-vars="GCS_BUCKET_NAME=unmute-datasets" \
...
The backend always reads datasets from GCS, in every environment (including local dev), so everyone works off the same data:
...
3. **vocab.json**: Loaded from `sgsl_processed/vocab.json` at startup via `read_json`. Token aliases (e.g. `PLS` -> `PLEASE`) live under the `aliases` key in this same file, generated by `scripts/build_vocab_from_json.py` from `scripts/aliases.json` and merged into the upload — so updating aliases just means editing `scripts/aliases.json`, rebuilding, and re-uploading `vocab.json`, with no backend redeploy needed.

## Local Development

Set environment variables so the backend can reach GCS locally:
